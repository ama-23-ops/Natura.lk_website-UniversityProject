<?php
session_start();
if (!isset($_SESSION['user_id'])) {
    header("Location: /login.php");
    exit();
}
include_once $_SERVER['DOCUMENT_ROOT'] . '/db.php';

// Fetch user details
$userId = $_SESSION['user_id'];
$stmt = $conn->prepare("SELECT * FROM users WHERE id = ?");
$stmt->execute([$userId]);
$user = $stmt->fetch(PDO::FETCH_ASSOC);

// Fetch recent orders with discount amounts
$stmt = $conn->prepare("SELECT * FROM orders WHERE user_id = ? ORDER BY order_date DESC LIMIT 5");
$stmt->execute([$userId]);
$recentOrders = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch user blogs
$stmt = $conn->prepare("SELECT * FROM blogs WHERE user_id = ? ORDER BY created_at DESC");
$stmt->execute([$userId]);
$userBlogs = $stmt->fetchAll(PDO::FETCH_ASSOC);
$totalBlogs = count($userBlogs);
$latestBlogDate = $totalBlogs > 0 ? date('M d, Y', strtotime($userBlogs[0]['created_at'])) : null;

// No need for discount calculation anymore since we're using values from database
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Customer Dashboard</title>
    <link href="/assets/css/style.css" rel="stylesheet">
    <style>
        .teal-gradient {
            background: linear-gradient(135deg, #00796b 0%, #009688 100%);
        }
        .card-hover:hover {
            transform: translateY(-5px);
            transition: all 0.3s ease;
        }
    </style>
</head>
<body class="bg-gray-50">
    <?php include $_SERVER['DOCUMENT_ROOT'] . '/customer/includes/header.php'; ?>
    <div class="container mx-auto p-6">
        <div class="teal-gradient rounded-lg p-6 mb-8 text-white shadow-lg">
            <div class="flex items-center">
                <i class="fas fa-user-circle text-4xl mr-4"></i>
                <div>
                    <h1 class="text-3xl font-bold">Welcome, <?= $user['name'] ?>!</h1>
                    <p class="text-teal-100">Manage your account and view recent orders</p>
                </div>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <!-- Profile Card -->
            <div class="bg-white rounded-lg shadow-md p-6 card-hover">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-semibold text-gray-800">Profile Details</h2>
                    <i class="fas fa-id-card text-teal-600 text-2xl"></i>
                </div>
                <div class="space-y-3">
                    <?php if (!empty($user['profile_picture'])): ?>
                    <div class="flex justify-center mb-4">
                        <img src="<?= $user['profile_picture'] ?>" alt="Profile Picture" class="rounded-full w-24 h-24 object-cover">
                    </div>
                    <?php endif; ?>
                    <p class="flex items-center text-gray-600">
                        <i class="fas fa-user mr-3 text-teal-500"></i>
                        <span class="font-medium"><?= $user['name'] ?></span>
                    </p>
                    <p class="flex items-center text-gray-600">
                        <i class="fas fa-envelope mr-3 text-teal-500"></i>
                        <span><?= $user['email'] ?></span>
                    </p>
                    <p class="flex items-center text-gray-600">
                        <i class="fas fa-phone mr-3 text-teal-500"></i>
                        <span><?= $user['contact_number'] ?></span>
                    </p>
                    <?php if (!empty($user['address'])): ?>
                    <p class="flex items-center text-gray-600">
                        <i class="fas fa-map-marker-alt mr-3 text-teal-500"></i>
                        <span><?= $user['address'] ?></span>
                    </p>
                    <?php endif; ?>
                    <div class="flex justify-center mt-4">
                        <a href="/customer/profile.php" class="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                            <i class="fas fa-edit mr-2"></i>Edit Profile
                        </a>
                    </div>
                </div>
            </div>

            <!-- Stats Card -->
            <div class="bg-white rounded-lg shadow-md p-6 card-hover">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-semibold text-gray-800">Account Stats</h2>
                    <i class="fas fa-chart-line text-teal-600 text-2xl"></i>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-3 bg-teal-50 rounded-lg">
                        <i class="fas fa-shopping-bag text-teal-500 text-xl mb-2"></i>
                        <p class="text-gray-600">Total Orders</p>
                        <p class="text-2xl font-bold text-teal-600"><?= count($recentOrders) ?></p>
                    </div>
                    <div class="text-center p-3 bg-teal-50 rounded-lg">
                        <i class="fas fa-clock text-teal-500 text-xl mb-2"></i>
                        <p class="text-gray-600">Member Since</p>
                        <p class="text-sm font-medium text-teal-600">
                            <?= isset($user['created_at']) ? date('M Y', strtotime($user['created_at'])) : 'N/A' ?>
                        </p>
                    </div>
                </div>
            </div>

            <!-- Blogs Card -->
            <div class="bg-white rounded-lg shadow-md p-6 card-hover">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-semibold text-gray-800">Blog Stats</h2>
                    <i class="fas fa-blog text-teal-600 text-2xl"></i>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-3 bg-teal-50 rounded-lg">
                        <i class="fas fa-file-alt text-teal-500 text-xl mb-2"></i>
                        <p class="text-gray-600">Total Blogs</p>
                        <p class="text-2xl font-bold text-teal-600"><?= $totalBlogs ?></p>
                    </div>
                    <?php if ($latestBlogDate): ?>
                    <div class="text-center p-3 bg-teal-50 rounded-lg">
                        <i class="fas fa-calendar-alt text-teal-500 text-xl mb-2"></i>
                        <p class="text-gray-600">Latest Blog</p>
                        <p class="text-sm font-medium text-teal-600"><?= $latestBlogDate ?></p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
        </div>

        <!-- Recent Orders Table -->
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-xl font-semibold text-gray-800">Recent Orders</h2>
                <i class="fas fa-shopping-cart text-teal-600 text-2xl"></i>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full">
                    <thead class="bg-teal-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-800 uppercase tracking-wider">
                                <i class="fas fa-hashtag mr-2"></i>Order ID
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-800 uppercase tracking-wider">
                                <i class="fas fa-calendar-alt mr-2"></i>Date
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-800 uppercase tracking-wider">
                                <i class="fas fa-dollar-sign mr-2"></i>Total Amount
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-800 uppercase tracking-wider">
                                <i class="fas fa-tag mr-2"></i>Discount
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-800 uppercase tracking-wider">
                                <i class="fas fa-receipt mr-2"></i>Net Amount
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-800 uppercase tracking-wider">
                                <i class="fas fa-info-circle mr-2"></i>Status
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-teal-800 uppercase tracking-wider">
                                <i class="fas fa-tasks mr-2"></i>Actions
                            </th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($recentOrders as $order) : ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">#<?= $order['id'] ?></td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    <?= date('M d, Y', strtotime($order['order_date'])) ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    $<?= number_format($order['total_amount'], 2) ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    $<?= number_format($order['discount_amount'], 2) ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    $<?= number_format($order['total_amount'] - $order['discount_amount'] + $order['tax_amount'], 2) ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        <?= $order['status'] === 'Completed' ? 'bg-green-100 text-green-800' : 
                                           ($order['status'] === 'Pending' ? 'bg-yellow-100 text-yellow-800' : 
                                            'bg-gray-100 text-gray-800') ?>">
                                        <i class="fas fa-circle text-xs mr-1 mt-1"></i>
                                        <?= $order['status'] ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <a href="/customer/order_details.php?id=<?= $order['id'] ?>" class="text-teal-600 hover:text-teal-800">
                                        <i class="fas fa-eye mr-1"></i>View Details
                                    </a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
    <?php include $_SERVER['DOCUMENT_ROOT'] . '/customer/includes/footer.php'; ?>
</body>
</html>